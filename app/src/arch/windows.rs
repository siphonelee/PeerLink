...
pub fn do_add_self_to_firewall_allowlist(inbound: bool) -> anyhow::Result<()> {
    ...
    let desc = BSTR::from("Allow PeerLink to do subnet proxy and kcp proxy");
    ...
    unsafe {
        ...
        rule.SetGrouping(&BSTR::from("PeerLink"))?;
...
fn add_protocol_firewall_rules(
    ...
    for (is_inbound, direction_name) in [(true, "Inbound"), (false, "Outbound")] {
        ...
        let description = format!(
            "Allow {} traffic on PeerLink interface {}",
...
fn add_all_protocols_firewall_rules(
    ...
    for (is_inbound, direction_name) in [(true, "Inbound"), (false, "Outbound")] {
        ...
        let rule_name = format!(
            "PeerLink {} - All Protocols ({})",
            interface_name, direction_name
        );
        let description = format!(
            "Allow all protocol traffic on PeerLink interface {}",
            interface_name
        );
...
/// List PeerLink firewall rules for specified interface (for debugging)
#[allow(dead_code)]
