...
#[derive(Parser, Debug)]
#[command(name = "peerlink-cli", author, version = PEERLINK_VERSION, about, long_about = None)]
struct Cli {
    /// the instance name
...
enum SubCommand {
    ...
    VpnPortal,
    #[command(about = "inspect self peerlink-core status")]
    Node(NodeArgs),
    #[command(about = "manage peerlink-core as a system service")]
    Service(ServiceArgs),
...
struct NetworkArgs {
    #[command(subcommand)]
    sub_command: Option<NetworkSubCommand>,
}
...
enum NetworkSubCommand {
    ...
    Create {
        #[arg(help = "Network name")]
        name: String,
        #[arg(help = "Network secret (minimum 8 characters)")]
        secret: String,
...
enum ServiceSubCommand {
    #[command(about = "register peerlink-core as a system service")]
    Install(InstallArgs),
    #[command(about = "unregister peerlink-core system service")]
    Uninstall,
    #[command(about = "check peerlink-core system service status")]
    Status,
    #[command(about = "start peerlink-core system service")]
    Start,
    #[command(about = "stop peerlink-core system service")]
    Stop,
}
...
struct InstallArgs {
    ...
    #[arg(long, help = "path to peerlink-core binary")]
    ...
    #[arg(
        ...
        help = "args to pass to peerlink-core"
    )]
...
impl CommandHandler<'_> {
        ...
        impl From<PeerRoutePair> for PeerTableItem {
            fn from(p: PeerRoutePair) -> Self {
                ...
                PeerTableItem {
                    ...
                    ipv4: route
                        .ipv4_addr
                        .map(|ip: peerlink::proto::common::Ipv4Inet| ip.address.unwrap_or_default())
                        .map(|ip| ip.to_string())
                        .unwrap_or_default(),
    ...
    async fn handle_network_list(&self) -> Result<(), Error> {
        tracing::info!("Listing networks from blockchain");
        ...
        let networks = chain_op.list_networks().await?;
        
        // Convert to display format with human-readable timestamps
        #[derive(tabled::Tabled, serde::Serialize)]
        struct NetworkDisplayItem {
            name: String,
            admin: String,
            created_at: String,
            description: String,
        ...
        let display_networks: Vec<NetworkDisplayItem> = networks
            .into_iter()
            .map(|network| NetworkDisplayItem {
                name: network.name,
                admin: network.admin,
                created_at: {
                    // Convert 13-digit timestamp (milliseconds) to human-readable format
                    let timestamp_ms = network.created_at;
                    let timestamp_secs = timestamp_ms / 1000;
                    match chrono::DateTime::from_timestamp(timestamp_secs as i64, ((timestamp_ms % 1000) * 1_000_000) as u32) {
                        Some(dt) => dt.format("%Y-%m-%d %H:%M:%S UTC").to_string(),
                        None => timestamp_ms.to_string(), // Fallback to raw timestamp if conversion fails
                    }
                },
        ...
        // Table format output
        print_output(&display_networks, self.output_format)?;
        
        Ok(())
    }

    async fn handle_network_info(&self, name: &str) -> Result<(), Error> {
        tracing::info!("Getting network info for '{}'", name);
    ...
    async fn handle_network_create(&self, name: String, secret: String, description: Option<String>) -> Result<(), Error> {
        tracing::info!("Creating network '{}' on blockchain", name);
        ...
        match chain_op.create_network(name.clone(), secret, description.clone()).await {
            Ok(response) => {
                if self.output_format == &OutputFormat::Json {
                    let result = serde_json::json!({
                        ...
                        "description": description.unwrap_or_else(|| "Network created via peerlink".to_string())
                    });
            ...
            Err(e) => {
                if self.output_format == &OutputFormat::Json {
                    ...
                    eprintln!("Failed to create network '{}': {}", name, e);
                }
                return Err(e.into());
            }
        }
...
async fn main() -> Result<(), Error> {
    ...
    match cli.sub_command {
        ...
        SubCommand::Network(network_args) => {
            match network_args.sub_command {
                Some(NetworkSubCommand::List) | None => {
                    handler.handle_network_list().await?;
                }
                Some(NetworkSubCommand::Info { name }) => {
                    handler.handle_network_info(&name).await?;
                }
...
"#,
            ...
            match service_args.sub_command {
                ServiceSubCommand::Install(install_args) => {
                    ...
                    let bin_path = std::fs::canonicalize(bin_path).map_err(|e| {
                        anyhow::anyhow!("failed to get peerlink core application: {}", e)
                    })?;
        ...
        SubCommand::GenAutocomplete { shell } => {
            ...
            peerlink::print_completions(shell, &mut cmd, "peerlink-cli");
        }
    }
...
mod win_service_manager {
    ...
    use peerlink::common::constants::WIN_SERVICE_WORK_DIR_REG_KEY;
