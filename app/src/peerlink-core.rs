...
#[derive(Parser, Debug)]
#[command(name = "peerlink-core", author, version = PEERLINK_VERSION , about, long_about = None)]
struct Cli {
    #[arg(
        short = 'w',
        long,
        env = "PL_CONFIG_SERVER",
...
struct NetworkOptions {
    ...
    #[arg(
        long,
        env = "PL_PROXY_FORWARD_BY_SYSTEM",
    ...
    #[arg(
        long,
        env = "PL_RELAY_NETWORK_WHITELIST",
        value_delimiter = ',',
    ...
    #[arg(
        long,
        env = "PL_DISABLE_UDP_HOLE_PUNCHING",
    ...
    #[arg(
        long,
        env = "PL_DISABLE_SYM_HOLE_PUNCHING",
    ...
    #[arg(
        long,
        env = "PL_FOREIGN_RELAY_BPS_LIMIT",
    ...
    #[arg(
        long,
        env = "PL_ENABLE_RELAY_FOREIGN_NETWORK_KCP",
...
async fn run_main(cli: Cli) -> anyhow::Result<()> {
    ...
    if cli.config_server.is_some() {
        ...
        let config_server_url = match url::Url::parse(&config_server_url_s) {
            Ok(u) => u,
            Err(_) => format!(
                "udp://config-server.peerlink.cn:22020/{}",
                config_server_url_s
            )
            .parse()
            .unwrap(),
        ...
        println!("Official config website: https://peerlink.cn/web");
...
async fn main() -> ExitCode {
    ...
    if let Some(shell) = cli.gen_autocomplete {
        ...
        peerlink::print_completions(shell, &mut cmd, "peerlink-core");
