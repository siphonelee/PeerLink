...
pub fn init_logger(
    ...
    if file_level != LevelFilter::OFF || need_reload {
        ...
        let file_appender = tracing_appender::rolling::Builder::new()
            ...
            .max_log_files(5)
            .filename_prefix(file_config.file.unwrap_or("peerlink".to_string()))
            .filename_suffix("log")
...
pub fn setup_panic_handler() {
    ...
    std::panic::set_hook(Box::new(|info| {
        ...
        let tmp_path = std::env::temp_dir().join("peerlink-panic.log");
        let candidate_path = [
            std::path::PathBuf::from_str("peerlink-panic.log").ok(),
            Some(tmp_path),
        ];
        let mut file = None;
        ...
        write_err("panic occurred, if this is a bug, please report this issue on github (https://github.com/peerlinkvpn/peerlink/issues)".to_string());
        write_err(format!("peerlink version: {}", crate::VERSION));
